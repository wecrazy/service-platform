...
func RenderDataTableServerSidePMBNI(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string, db *gorm.DB) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "string":
			allowedCols := map[string]bool{
				...
				"source":             true,
				"message_cc":         true,
				"status_merchant":    true,
				"wo_remark_tiket":    true,
				"longitude":          true,
				"latitude":           true,
				"link_photo":         true,
				"ticket_type":        true,
		...
		case "time.Time", "*time.Time":
			...
			allowedCols := map[string]bool{
				"sla_deadline":           true,
				"create_date":            true,
				"received_datetime_spk":  true,
				"plan_date":              true,
				"timesheet_last_stop":    true,
				"date_last_stage_update": true,
			}
...
`
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_data_pm_bni.html", map[string]any{
...
func RenderDataTableServerSideNonPMBNI(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string, db *gorm.DB) template.HTML {
	var column_array []int
	for i, col := range table_columns {
		if col.Visible {
			column_array = append(column_array, i)
...
`
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_data_non_pm_bni.html", map[string]any{
		"title":           template.HTML(title),
		"table_name":      table_name,
		"endpoint":        template.URL(endpoint),
		"table_columns":   table_columns,
