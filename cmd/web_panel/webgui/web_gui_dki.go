...
func RenderDataTableServerSideTicketDKI(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string, db *gorm.DB) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "string":
			allowedCols := map[string]bool{
				...
				"merchant_city":          true,
				"merchant_zip_code":      true,
				"merchant_address":       true,
				"merchant_condition":     true,
				"edc_status":             true,
				"edc_condition":          true,
				"wo_remark":              true,
				"ticket_type":            true,
				...
				"edc_type":               true,
				"sla_status":             true,
				"first_task_reason":      true,
				"first_task_reason_code": true,
				"first_task_message":     true,
			}
...
`
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_data_ticket_dki.html", map[string]any{
