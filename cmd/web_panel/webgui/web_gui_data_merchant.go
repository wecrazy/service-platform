...
func RenderDataTableServerSideDataMerchant(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "string":
			...
			// // Tabel Data
			// if title == "Data Merchant" | Data Merchant FastLink {
			// 	allowedCols := map[string]bool{
			// 		"spk_assigned_at":             true,
			// 		"spk_completed":               true,
			// 		"task_to_be_visit_planned_on": true,
			// 		"customer":                    true,
			// 		"stage":                       true,
			// 		"id_atm":                      true,
			// 	}
			...
			if title == "Data Merchant FastLink" {
				allowedCols := map[string]bool{
					"status": true,
				}
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_data_merchant.html", map[string]any{
