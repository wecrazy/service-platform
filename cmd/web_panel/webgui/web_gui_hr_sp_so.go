...
func RenderDataTableServerSideSPSO(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string, db *gorm.DB) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "time.Time", "*time.Time":
			...
			allowedCols := map[string]bool{
				...
				"got_sp2_at":          true,
				"got_sp3_at":          true,
				"sp1_sound_played_at": true,
				"sp2_sound_played_at": true,
				"sp3_sound_played_at": true,
			}
...
`
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_sp_so.html", map[string]any{
