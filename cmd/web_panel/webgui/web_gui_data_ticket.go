...
func RenderDataTableServerSideDataTicket(title, table_name, endpoint string, page_length int, length_menu []int, order []any, table_columns []Column, insertable, editable, deletable, hideHeader, passwordable bool, scrollUpDown, scrollLeftRight bool, exportType []string, db *gorm.DB) template.HTML {
	...
	for i, col := range table_columns {
		...
		case "string":
			...
			if table_columns[i].SelectableSrc != "" {
				...
				case "ticket_type":
					var dataTicketType []model.TicketType

					if err := db.Find(&dataTicketType).Error; err != nil {
					...
					optionsHTML := `<option value="" disabled selected hidden>-- Select Ticket Type --</option>`
					for _, ticketType := range dataTicketType {
						optionsHTML += fmt.Sprintf(`<option value="%s">%s</option>`, ticketType.Name, ticketType.Name)
					}
					...
					// Add filter dropdown for ticket_type
					filterOptionsHTML := `<option value="">-- All Ticket Types --</option>`
					for _, ticketType := range dataTicketType {
						filterOptionsHTML += fmt.Sprintf(`<option value="%s">%s</option>`, ticketType.Name, ticketType.Name)
					}
					table_columns[i].Filter = template.HTML(fmt.Sprintf(
						`<label class="form-label">%s:</label>
						<select
						...
						col.Header, filter_id, i, i-1, filterOptionsHTML))
				case "priority":
					table_columns[i].InsertField = template.HTML(fmt.Sprintf(`
					<label class="form-label">%s:</label>
					<input type="hidden" name="%s" id="ratingValue" value="0">

					<div id="starRating" style="font-size: 24px; cursor: pointer;">
						<i class="fal fa-star text-secondary" data-value="1"></i>
						<i class="fal fa-star text-secondary" data-value="2"></i>
						<i class="fal fa-star text-secondary" data-value="3"></i>
					</div>
					...
					document.querySelectorAll(".datetime-input").forEach(function(input) {
						...
						// Manually trigger flatpickr when clicking the calendar icon
						const trigger = input.parentElement.querySelector(".calendar-trigger");
						if (trigger) {
					...
					$(document).ready(function () {
						...
						const $starsEdit = $('#starRatingEdit i');
						const $ratingInputEdit = $('#ratingValueEdit');

						$stars.on('click', function () {
							const clickedValue = parseInt($(this).data('value'));
							const currentValue = parseInt($ratingInput.val()) || 0;

							const newRating = clickedValue === currentValue ? 0 : clickedValue;
						...
						$starsEdit.on('click', function () {
							const clickedValue = parseInt($(this).data('value'));
							const currentValue = parseInt($ratingInputEdit.val()) || 0;

							const newRating = clickedValue === currentValue ? 0 : clickedValue;
							$ratingInputEdit.val(newRating);

							$starsEdit.each(function (index) {
							if (index < newRating) {
								$(this).removeClass('fal').addClass('fas');
							} else {
								$(this).removeClass('fas').addClass('fal');
					...
					table_columns[i].EditForm = template.HTML(fmt.Sprintf(`
					<label class="form-label">%s:</label>
					<input type="hidden" name="%s" id="ratingValueEdit" value="0">

					<div id="starRatingEdit" style="font-size: 24px; cursor: pointer;">
						<i class="fal fa-star text-secondary" data-value="1"></i>
						<i class="fal fa-star text-secondary" data-value="2"></i>
						<i class="fal fa-star text-secondary" data-value="3"></i>
					</div>
					...
					id="%s"
					type="number"
					class="form-control dt-input dt-priority"
					data-column="%d"
					placeholder="%s (0-3) ⭐"
					data-column-index="%d"
					min="0"
					max="3"
					step="1" />`, col.Header, filter_id, i, col.Header, i-1))
				default:
					table_columns[i].InsertField = template.HTML(fmt.Sprintf(`
					<label class="form-label">%s:</label>
					<input
	...
	renderedHTML, err := RenderTemplateToString("gui_server_table_data_ticket.html", map[string]any{
