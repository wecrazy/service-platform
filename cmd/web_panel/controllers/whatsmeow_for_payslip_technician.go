...
var (
	WhatsappEventMsgForPayslipTechnician      *events.Message
	WhatsappEventsReceiptForPayslipTechnician *events.Receipt
)

func handleRepliesReactionsAndEditedMsgForPayslipTechnician() {
	e := WhatsappEventMsgForPayslipTechnician
	...
	if ctxInfo != nil && ctxInfo.QuotedMessage != nil && ctxInfo.StanzaID != nil && *ctxInfo.StanzaID != "" {
		...
		var dataEDC odooms.MSTechnicianPayroll
		var dataDedicated odooms.MSTechnicianPayrollDedicatedATM
		var useDedicated bool

		if err := dbWeb.
			Where("whatsapp_chat_id = ?", stanzaID).
			First(&dataEDC).Error; err != nil {
			// Try dedicated ATM table
			if err2 := dbWeb.
				Where("whatsapp_chat_id = ?", stanzaID).
				First(&dataDedicated).Error; err2 != nil {
				// logrus.Errorf("No data matched for whatsapp id %s: %v", stanzaID, err)
				return
			}
			useDedicated = true
		}

		var updatedDBData odooms.MSTechnicianPayroll
		var repliedAt *time.Time
		t := time.Now()
		repliedAt = &t

		updatedDBData = odooms.MSTechnicianPayroll{
	...
	if r := e.Message.GetReactionMessage(); r != nil {
		...
		var tech odooms.MSTechnicianPayroll
		var techDedicated odooms.MSTechnicianPayrollDedicatedATM
		var useDedicated bool
		...
		if err := dbWeb.
			...
			if err2 := dbWeb.
				Where("whatsapp_chat_id = ?", stanzaID).
				First(&techDedicated).Error; err2 != nil {
				logrus.Errorf("No data matched for whatsapp id %s: %v", stanzaID, err)
				return
			}
			useDedicated = true
	...
	if pm := e.Message.GetProtocolMessage(); pm != nil {
		if pm.GetType() == waE2E.ProtocolMessage_MESSAGE_EDIT {
			...
			if err := dbWeb.
				Where("whatsapp_chat_id = ?", messageIDToUpdate).
				First(&tech).Error; err != nil {
				// Try dedicated ATM table
				if err2 := dbWeb.
					Where("whatsapp_chat_id = ?", messageIDToUpdate).
					First(&techDedicated).Error; err2 != nil {
					logrus.Errorf("No data matched for whatsapp id %s: %v", messageIDToUpdate, err)
					return
				}
				useDedicated = true
			}
...
func handleReceiptForPayslipTechnician() {
	e := WhatsappEventsReceiptForPayslipTechnician
	dbWeb := gormdb.Databases.Web

	for _, msgID := range e.MessageIDs {
		if string(e.Type) != "" {
			var data odooms.MSTechnicianPayroll
			var dataDedicated odooms.MSTechnicianPayrollDedicatedATM
			var useDedicated bool
			...
			if useDedicated {
				if err := dbWeb.Model(&odooms.MSTechnicianPayrollDedicatedATM{}).
					...
					logrus.Errorf("Failed to update whatsapp_msg_status for technician %s: %v", dataDedicated.Name, err)
					continue
				}
			} else {
				if err := dbWeb.Model(&odooms.MSTechnicianPayroll{}).
					Where("whatsapp_chat_id = ?", msgID).
					Update("whatsapp_msg_status", string(e.Type)).Error; err != nil {
					logrus.Errorf("Failed to update whatsapp_msg_status for technician %s: %v", data.Name, err)
					continue
				}
			}
		}
...
func sendLangDocumentMessageForPayslipTechnician(projMS, technician, jid, idmsg, enmsg, lang, filePath string) {
	...
	sendDocumentViaBotForPayslipTechnician(projMS, technician, jid, msg, filePath)
}

func sendDocumentViaBotForPayslipTechnician(projMS, technician, jid, message, filePath string) {
	dbWeb := gormdb.Databases.Web
