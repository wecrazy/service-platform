...
var (
	WhatsappEventsMsgForSPTechnician     *events.Message
	WhatsappEventsReceiptForSPTechnician *events.Receipt
)

func handleRepliesReactionsAndEditedMsgForSPTechnician(forProject string) {
	e := WhatsappEventsMsgForSPTechnician
	...
	if ctxInfo != nil && ctxInfo.QuotedMessage != nil && ctxInfo.StanzaID != nil && *ctxInfo.StanzaID != "" {
		...
		if result.Error != nil {
			...
			logrus.Errorf("Database error while fetching SPWhatsAppMessage for whatsapp_chat_id '%s', project '%s', type 'SP_TECHNICIAN': %v", stanzaID, forProject, result.Error)
			return
		}
		...
		// For the notification, we need details from the parent TechnicianGotSP record
		...
		if spMessage.TechnicianGotSPID != nil {
			if err := dbWeb.First(&tech, *spMessage.TechnicianGotSPID).Error; err == nil {
				...
				textID := fmt.Sprintf("🙏🏽 Terimakasih atas respon Anda untuk SP-%d teknisi %s yang dikirim pada %v.",
					spMessage.NumberOfSP,
					tech.Technician,
					sentAt,
				)
				textEN := fmt.Sprintf("🙏🏽 Thank you for your reply to SP-%d for technician %s sent at %v.",
					spMessage.NumberOfSP,
					tech.Technician,
					sentAt,
				)
	...
	if r := e.Message.GetReactionMessage(); r != nil {
		...
		if err := dbWeb.Where("whatsapp_chat_id = ? AND for_project = ? AND what_sp = ?", stanzaID, forProject, whatSP).First(&spMessage).Error; err != nil {
			logrus.Warnf("No SPWhatsAppMessage matched for reaction on whatsapp_chat_id '%s', project '%s', type 'SP_TECHNICIAN': %v", stanzaID, forProject, err)
			return
		}
	...
	if pm := e.Message.GetProtocolMessage(); pm != nil {
		if pm.GetType() == waE2E.ProtocolMessage_MESSAGE_EDIT {
			...
			if err := dbWeb.Where("whatsapp_chat_id = ? AND for_project = ? AND what_sp = ?", messageIDToUpdate, forProject, whatSP).First(&spMessage).Error; err != nil {
				logrus.Warnf("No SPWhatsAppMessage matched for edited message on whatsapp_chat_id '%s', project '%s', type 'SP_TECHNICIAN': %v", messageIDToUpdate, forProject, err)
				return
			}
			...
			if spMessage.TechnicianGotSPID != nil {
				if err := dbWeb.First(&tech, *spMessage.TechnicianGotSPID).Error; err == nil {
					...
					// Send replied text to Whatsapp HRD
					idText := fmt.Sprintf("SP-%d untuk %s yang dikirim pada %v mendapatkan respon (edit) dari %s, yakni %s",
						spMessage.NumberOfSP,
						tech.Technician,
						sentAt,
						userReplied,
					...
					enText := fmt.Sprintf("SP-%d for %s sent at %v received a reply (edit) from %s, which is %s",
						spMessage.NumberOfSP,
						tech.Technician,
						sentAt,
						userReplied,
...
func handleReceiptForSPTechnician(forProject string) {
	e := WhatsappEventsReceiptForSPTechnician
	dbWeb := gormdb.Databases.Web
	whatSP := "SP_TECHNICIAN"
...
func sendLangDocumentMessageForSPTechnician(
	forProject,
	technician,
	jid,
	idmsg,
	...
	sendDocumentViaBotForSPTechnician(forProject, technician, jid, msg, filePath, spNumber, spSentTo)
}

func sendDocumentViaBotForSPTechnician(forProject, technician, jid, message, filePath string, spNumber int, spSentTo string) {
	dbWeb := gormdb.Databases.Web
	...
	// // If sending failed, resp will be nil. We should exit the goroutine.
	// if resp == nil {
	// 	logrus.Error("Cannot save WhatsApp message to DB because the send operation failed and response is nil.")
	// 	return
	// }

	go func() {
		// 1. Find the parent TechnicianGotSP record
		var techSP sptechnicianmodel.TechnicianGotSP
		if err := dbWeb.
			Where("for_project = ? AND technician = ?", forProject, technician).
			First(&techSP).Error; err != nil {
			logrus.Errorf("Failed to find TechnicianGotSP for %s/%s to link whatsapp message: %v", forProject, technician, err)
			return
		}
		...
		newSPMessage := sptechnicianmodel.SPWhatsAppMessage{
			TechnicianGotSPID:     &techSP.ID,
			WhatSP:                whatSP,
		...
		// 3. Save the new message record to the database
		if err := dbWeb.Create(&newSPMessage).Error; err != nil {
			logrus.Errorf("Failed to create SPWhatsAppMessage for %s/%s: %v", forProject, technician, err)
			return
		}

		logrus.Infof("Successfully saved SP %d WhatsApp message (that send to %s) for technician %s to the database.", spNumber, spSentTo, technician)
	}()
}

// UnboldedLinkWAMsg takes a string and makes all non-URL text bold (*...*),
// while leaving URLs as plain text.
func UnboldedLinkWAMsg(t string) string {
	urlPattern := `(https?://[\w\-\.\:]+(?:/[\w\-\.\?\=\&%#\-/]*)?)`
	re := regexp.MustCompile(urlPattern)
	urls := re.FindAllStringIndex(t, -1)

	// If no URLs → just bold the whole message
	if len(urls) == 0 {
		clean := strings.ReplaceAll(strings.ReplaceAll(t, "*", ""), "_", "")
		return "*" + clean + "*"
	}
	...
	for _, m := range urls {
		...
		// Bold text before the URL
		if start > last {
			chunk := t[last:start]
			chunk = strings.ReplaceAll(strings.ReplaceAll(chunk, "*", ""), "_", "")
		...
		// URL stays plain
		sb.WriteString(t[start:end])
		sb.WriteString(" ")

		last = end
	}

	// Bold the tail after the last URL
	if last < len(t) {
		chunk := t[last:]
		chunk = strings.ReplaceAll(strings.ReplaceAll(chunk, "*", ""), "_", "")
		if strings.TrimSpace(chunk) != "" {
