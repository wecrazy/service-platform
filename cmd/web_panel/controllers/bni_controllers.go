...
var (
	getDataTaskODOOMSBNIMutex sync.Mutex

	regionCacheBNIMutex       sync.RWMutex
	regionCacheBNI            map[string]string
	regionCacheBNIInitialized bool
)

func initRegionCacheBNI() {
	regionCacheBNIMutex.Lock()
	defer regionCacheBNIMutex.Unlock()

	if regionCacheBNIInitialized {
		return
	}

	logrus.Info("Initializing BNI region cache...")
	...
	regionCacheBNI = make(map[string]string, 1000)
	for _, region := range allRegions {
		province := region.Province
		// Index by district
		if region.District != "" {
			regionCacheBNI[strings.ToLower(region.District)] = province
		}
		// Index by subdistrict
		if region.Subdistrict != "" {
			regionCacheBNI[strings.ToLower(region.Subdistrict)] = province
		}
		// Index by area
		if region.Area != "" {
			regionCacheBNI[strings.ToLower(region.Area)] = province
		}
		// Index by post code
		if region.PostCode != "" {
			regionCacheBNI[strings.ToLower(region.PostCode)] = province
		}
	}
	regionCacheBNIInitialized = true
	logrus.Infof("BNI region cache initialized with %d entries", len(regionCacheBNI))
}

func getProvinceFromCityBNI(city string) string {
	if !regionCacheBNIInitialized {
		initRegionCacheBNI()
	}

	regionCacheBNIMutex.RLock()
	defer regionCacheBNIMutex.RUnlock()
	if city == "" {
		return "N/A"
	}

	cityLower := strings.ToLower(city)
	if province, exists := regionCacheBNI[cityLower]; exists {
		return province
	}
...
func getSACFromTechnicianBNI(technicianName string) string {
	if technicianName == "" {
		return "N/A"
	}
...
func GetTaskODOOMSBNI() error {
	taskDoing := "Get BNI Task Data from ODOO MS"
	startTime := time.Now()

	if !getDataTaskODOOMSBNIMutex.TryLock() {
		return fmt.Errorf("%s is still running, please wait until the previous process is finished", taskDoing)
	}
	defer getDataTaskODOOMSBNIMutex.Unlock()
	...
	if len(ids) == 0 {
		return errors.New("no BNI task IDs found in ODOO MS response")
	}

	logrus.Infof("Got BNI Task %d IDs", len(ids))
	...
	dbWeb := gormdb.Databases.Web
	result := dbWeb.Unscoped().Where("1=1").Delete(&bnimodel.BNIOdooMSData{})
	if result.Error != nil {
		return fmt.Errorf("failed to clear existing BNI ODOO MS data: %v", result.Error)
	}
	logrus.Infof("Cleared %d existing BNI ODOO MS records", result.RowsAffected)
	...
	for result := range resultChan {
		...
		go func(chunkData []OdooTaskDataRequestItem, chunkIndex int) {
			...
			for i := 0; i < len(chunkData); i += dbBatchSize {
				...
				var batch []bnimodel.BNIOdooMSData
				for _, data := range chunkData[i:end] {
					...
					batch = append(batch, bnimodel.BNIOdooMSData{
						ID:                  uint(data.ID),
				...
				if len(batch) > 0 {
					if err := dbWeb.Model(&bnimodel.BNIOdooMSData{}).Create(&batch).Error; err != nil {
						errorChan <- fmt.Errorf("failed to insert BNI ODOO MS data batch for chunk %d: %v", chunkIndex, err)
						return
					}
	...
	logrus.Infof("Inserted %d new BNI ODOO MS records successfully, memory usage: Alloc=%dMB Sys=%dMB NumGC=%d", totalRecords, memStats.Alloc/1024/1024, memStats.Sys/1024/1024, memStats.NumGC)
...
func RefreshTicketBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		err := GetTaskODOOMSBNI()
		if err != nil {
...
func GetLastUpdateTicketBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var result struct {
		...
		err := dbWeb.Model(&bnimodel.BNIOdooMSData{}).
...
func TablePMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
			Draw       int    `form:"draw"`
		...
		dbWeb := gormdb.Databases.Web
		t := reflect.TypeOf(bnimodel.BNIOdooMSData{})
		...
		filteredQuery := dbWeb.Model(&bnimodel.BNIOdooMSData{})
		...
		var totalRecords int64
		dbWeb.Model(&bnimodel.BNIOdooMSData{}).Where(&bnimodel.BNIOdooMSData{TaskType: "Preventive Maintenance"}).Count(&totalRecords)

		// Count the number of filtered records
		var filteredRecords int64
		filteredQuery.Where(&bnimodel.BNIOdooMSData{TaskType: "Preventive Maintenance"}).Count(&filteredRecords)

		// Apply sorting and pagination to the filtered query
		query := filteredQuery.Order(orderString)
		var Dbdata []bnimodel.BNIOdooMSData
				...
				case "link_photo":
					newData[theKey] =
						fmt.Sprintf(
							`
							<div class="card-cek">
								<button class="btn btn-md btn-label-info" onclick="openPopupODOOMSPhotos('%d', 'task-bni')">
...
func PivotPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
			Draw       int    `form:"draw"`
			Start      int    `form:"start"`
			...
			// Add all the same form fields as TablePMBNI
		...
		// Build WHERE clause with filters (same logic as TablePMBNI)
		...
		query := `
			SELECT 
				...
				COUNT(DISTINCT CASE WHEN stage NOT IN ('New', 'Cancel') THEN tid END) as terkunjungi_count,
...
func ReportAllPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		dbWeb := gormdb.Databases.Web
		startTime := time.Now()

		logrus.Info("Starting generation of All PM BNI Excel report with optimizations")

		// First, check the count to see if we have data
		var totalCount int64
		if err := dbWeb.Model(&bnimodel.BNIOdooMSData{}).Where("task_type = ?", "Preventive Maintenance").Count(&totalCount).Error; err != nil {
			logrus.Errorf("Error counting PM BNI data: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to count PM BNI data",
			})
			return
		}

		if totalCount == 0 {
			logrus.Warn("No PM BNI data found")
			c.JSON(http.StatusNotFound, gin.H{
				"success": false,
				"error":   "No PM BNI data available for report generation",
			})
			return
		}

		logrus.Infof("Found %d PM BNI records, proceeding with parallel processing", totalCount)
		...
		type result struct {
			allData   []bnimodel.BNIOdooMSData
		...
		go func() {
			...
			allData, err := fetchPMBNIDataBatched(ctx, dbWeb, totalCount)
		...
		for i := 0; i < 2; i++ {
			...
			case <-ctx.Done():
				logrus.Error("Timeout while fetching PM BNI data and generating pivot")
		...
		if dataFetchErr != nil {
			logrus.Errorf("Error fetching PM BNI data: %v", dataFetchErr)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to fetch PM BNI data",
			})
			return
		}
		...
		excelFile, err := createExcelReportPMBNIOptimized(allData, pivotData, "All PM BNI Data")
		if err != nil {
		...
		logrus.Infof("Successfully generated and served PM BNI Excel report: %s", filename)
	}
}

func fetchPMBNIDataBatched(ctx context.Context, dbWeb *gorm.DB, totalCount int64) ([]bnimodel.BNIOdooMSData, error) {
	const batchSize = 5000 // Process 5000 records at a time
	var allData []bnimodel.BNIOdooMSData

	// Pre-allocate slice with known capacity for better memory management
	allData = make([]bnimodel.BNIOdooMSData, 0, totalCount)
	...
	for i := 0; i < numBatches; i++ {
		...
		go func(batchNum int) {
			...
			var batchData []bnimodel.BNIOdooMSData
	...
	batchResults := make(map[int][]bnimodel.BNIOdooMSData)
...
func generatePivotDataPMBNI(dbWeb *gorm.DB, searchValue string) ([]map[string]interface{}, error) {
	...
	query := fmt.Sprintf(`
		SELECT 
			...
			SUM(CASE WHEN stage NOT IN ('New', 'Cancel') THEN 1 ELSE 0 END) as terkunjungi_count,
			ROUND(SUM(CASE WHEN stage NOT IN ('New', 'Cancel') THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) as terkunjungi_percentage,
			SUM(CASE WHEN stage = 'Cancel' THEN 1 ELSE 0 END) as gagal_terkunjungi_count,
			ROUND(SUM(CASE WHEN stage = 'Cancel' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) as gagal_terkunjungi_percentage,
			SUM(CASE WHEN stage = 'New' THEN 1 ELSE 0 END) as belum_kunjungan_count,
			ROUND(SUM(CASE WHEN stage = 'New' THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) as belum_kunjungan_percentage,
			COUNT(*) as total_count,
			SUM(CASE WHEN stage NOT IN ('New', 'Cancel') THEN 1 ELSE 0 END) as run_rate,
			ROUND(SUM(CASE WHEN stage NOT IN ('New', 'Cancel') THEN 1 ELSE 0 END) * 100.0 / %d, 2) as run_rate_percentage,
			%d as target_per_hari
		FROM %s 
		WHERE %s
		GROUP BY source
		ORDER BY source`,
		targetPerHari, targetPerHari, tableBNI, whereClause)
...
func createExcelReportPMBNIOptimized(masterData []bnimodel.BNIOdooMSData, pivotData []map[string]interface{}, reportTitle string) (*excelize.File, error) {
...
func ReportDataFilteredPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		timeStart := time.Now()
		dbWeb := gormdb.Databases.Web

		logrus.Info("Starting generation of Filtered PM BNI Excel report with optimizations")
		...
		filteredQuery := dbWeb.Model(&bnimodel.BNIOdooMSData{}).Where("task_type = ?", "Preventive Maintenance")
		...
		if err := filteredQuery.Count(&countDataFiltered).Error; err != nil {
			logrus.Errorf("Error counting filtered PM BNI data: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to count filtered PM BNI data",
			})
			return
		}

		logrus.Infof("Found %d filtered PM BNI records to process", countDataFiltered)
		...
		var wg sync.WaitGroup
		var filteredData []bnimodel.BNIOdooMSData
		var pivotData []map[string]interface{}
		var dataErr error
		...
		go func() {
			...
			if countDataFiltered > 10000 {
				...
				filteredData, dataErr = fetchFilteredPMBNIDataBatched(ctx, dbWeb, c.Request.Form, c, countDataFiltered)
			} else {
		...
		if dataErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to fetch filtered PM BNI data",
			})
			return
		}
		...
		pivotData = generatePivotDataFromFilteredDataBNI(filteredData)
		...
		excelFile, err := createExcelReportPMBNIOptimized(filteredData, pivotData, "Filtered PM BNI Data")
		if err != nil {
			logrus.Errorf("Error creating Excel file: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
		...
		logrus.Infof("Successfully generated and served filtered PM BNI Excel report: %s in %v", filename, duration)
	}
}

func fetchFilteredPMBNIDataBatched(ctx context.Context, dbWeb *gorm.DB, formData map[string][]string, c *gin.Context, totalCount int64) ([]bnimodel.BNIOdooMSData, error) {
...
func generatePivotDataFromFilteredDataBNI(filteredData []bnimodel.BNIOdooMSData) []map[string]interface{} {
	const targetPerHari = 12 // Default target, make configurable if needed
...
func TableNonPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
			Draw       int    `form:"draw"`
			Start      int    `form:"start"`
		...
		// Count the total number of records
		var totalRecords int64
		dbWeb.Model(&bnimodel.BNIOdooMSData{}).Where("task_type != ?", "Preventive Maintenance").Count(&totalRecords)
...
func PivotNonPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
			Draw       int    `form:"draw"`
			Start      int    `form:"start"`
			...
			// Add all the same form fields as TableNonPMBNI
		...
		// Build WHERE clause with filters (same logic as TableNonPMBNI)
		whereConditions := []string{"task_type != 'Preventive Maintenance'", "deleted_at IS NULL"}
		args := []interface{}{}
		...
		var rawData []bnimodel.BNIOdooMSData
		...
		logrus.Debugf("PivotNonPMBNI: Found %d raw records to process", len(rawData))
		...
		for _, data := range rawData {
			...
			province := getProvinceFromCityBNI(data.MerchantCity)
			if province == "" {
				province = getProvinceFromCityBNI(data.MerchantZip)
			}
			...
			sac := getSACFromTechnicianBNI(technician)
		...
		logrus.Debugf("PivotNonPMBNI: Processed %d records, nullTimesheet=%d, validSLA=%d, nullSLA=%d, missedSLA=%d",
...
func ReportAllNonPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		timeStart := time.Now()
		dbWeb := gormdb.Databases.Web

		logrus.Info("Starting generation of All Non-PM BNI Excel report with optimizations")

		// Get count first to determine processing strategy
		var totalCount int64
		if err := dbWeb.Model(&bnimodel.BNIOdooMSData{}).Where("task_type != ?", "Preventive Maintenance").Count(&totalCount).Error; err != nil {
			logrus.Errorf("Error counting Non-PM BNI records: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to count Non-PM BNI records",
			})
			return
		}

		logrus.Infof("Found %d Non-PM BNI records to process", totalCount)

		if totalCount == 0 {
			logrus.Info("No Non-PM BNI data found")
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"error":   "No Non-PM BNI data found",
			})
			return
		}
		...
		go func() {
			...
			if totalCount > 10000 {
				// Use batched approach for large datasets
				allData, dataErr = fetchNonPMBNIDataBatched(ctx, dbWeb, totalCount)
			} else {
		...
		go func() {
			defer wg.Done()
			pivotData, pivotErr = generatePivotDataNonPMBNI(dbWeb, "")
			if pivotErr == nil {
		...
		if dataErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to fetch Non-PM BNI data",
			})
			return
		}
		...
		// Create Excel file using optimized function
		excelFile, err := createExcelReportNonPMBNIOptimized(allData, pivotData, "All Non-PM BNI Data")
		if err != nil {
			logrus.Errorf("Error creating Excel file: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
		...
		duration := time.Since(timeStart)
		logrus.Infof("Successfully generated and served Non-PM BNI Excel report: %s in %v", filename, duration)
	}
}

func fetchNonPMBNIDataBatched(ctx context.Context, dbWeb *gorm.DB, totalCount int64) ([]bnimodel.BNIOdooMSData, error) {
	const batchSize = 5000
	var allData []bnimodel.BNIOdooMSData
...
func generatePivotDataNonPMBNI(dbWeb *gorm.DB, searchValue string) ([]map[string]interface{}, error) {
	whereClause := "task_type != 'Preventive Maintenance'"
	var args []interface{}
	...
	for _, record := range rawData {
		...
		province := getProvinceFromCityBNI(record.MerchantCity)
		...
		// sac := getSACFromTechnicianBNI(record.Technician)
		spLeader := "N/A" // You may need to implement getSPLeaderFromTechnicianBNI if needed
...
func createExcelReportNonPMBNIOptimized(masterData []bnimodel.BNIOdooMSData, pivotData []map[string]interface{}, reportTitle string) (*excelize.File, error) {
	_ = reportTitle // Currently unused, can be used for metadata if needed
...
func ReportDataFilteredNonPMBNI() gin.HandlerFunc {
	return func(c *gin.Context) {
		timeStart := time.Now()
		dbWeb := gormdb.Databases.Web

		logrus.Info("Starting generation of Filtered Non-PM BNI Excel report with optimizations")
		...
		var countDataFiltered int64
		if err := filteredQuery.Count(&countDataFiltered).Error; err != nil {
			logrus.Errorf("Error counting filtered Non-PM BNI data: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to count filtered Non-PM BNI data",
			})
			return
		}

		logrus.Infof("Found %d filtered Non-PM BNI records to process", countDataFiltered)
		...
		go func() {
			...
			if countDataFiltered > 10000 {
				// Use batched approach for large datasets
				filteredData, dataErr = fetchFilteredNonPMBNIDataBatched(ctx, dbWeb, c.Request.Form, c, countDataFiltered)
			} else {
				// Direct query for smaller datasets using same logic as batched approach
				filteredData, dataErr = fetchFilteredNonPMBNIDataBatched(ctx, dbWeb, c.Request.Form, c, countDataFiltered)
			}
		...
		if dataErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
				"error":   "Failed to fetch filtered Non-PM BNI data",
			})
			return
		}
		...
		// Generate pivot data from the same filtered data to ensure consistency
		pivotData = generatePivotDataFromFilteredDataNonPMBNI(filteredData)
		logrus.Infof("Generated Non-PM pivot data with %d entries from %d filtered records", len(pivotData), len(filteredData))

		// Create Excel file using optimized function
		excelFile, err := createExcelReportNonPMBNIOptimized(filteredData, pivotData, "Filtered Non-PM BNI Data")
		if err != nil {
			logrus.Errorf("Error creating Excel file: %v", err)
			c.JSON(http.StatusInternalServerError, gin.H{
				"success": false,
		...
		duration := time.Since(timeStart)
		logrus.Infof("Successfully generated and served filtered Non-PM BNI Excel report: %s in %v", filename, duration)
	}
}

func fetchFilteredNonPMBNIDataBatched(ctx context.Context, dbWeb *gorm.DB, formData map[string][]string, c *gin.Context, totalCount int64) ([]bnimodel.BNIOdooMSData, error) {
	_ = c // Unused parameter, kept for potential future use
	const batchSize = 5000
	var allData []bnimodel.BNIOdooMSData
...
func generatePivotDataFromFilteredDataNonPMBNI(filteredData []bnimodel.BNIOdooMSData) []map[string]interface{} {
	const targetPerHari = 12 // Default target, make configurable if needed
