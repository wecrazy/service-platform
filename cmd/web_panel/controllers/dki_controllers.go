...
var (
	getTicketDKIODOOMSMutex sync.Mutex
)

func GetTicketDKIODOOMS() (string, error) {
	taskDoing := "Get Ticket DKI"
	if !getTicketDKIODOOMSMutex.TryLock() {
		return "", fmt.Errorf("%s task is still running. Please wait until it is finished", taskDoing)
	}
	defer getTicketDKIODOOMSMutex.Unlock()
	...
	// Clear existing records for DKI from the database
	dbWeb := gormdb.Databases.Web
	result := dbWeb.Unscoped().Where("1=1").Delete(&dkimodel.TicketDKI{})
	if result.Error != nil {
		return "", fmt.Errorf("failed to clear existing DKI tickets: %v", result.Error)
	}
	logrus.Infof("Cleared %d existing DKI tickets from the database", result.RowsAffected)
	...
	for i := 0; i < len(listOfData); i += dbBatchSize {
		...
		for _, data := range listOfData[i:end] {
			...
			batch = append(batch, dkimodel.TicketDKI{
				ID:                        data.ID,
		...
		if err := dbWeb.Model(&dkimodel.TicketDKI{}).Create(&batch).Error; err != nil {
			return "", fmt.Errorf("failed to insert DKI tickets batch starting at index %d: %v", i, err)
		}
	}

	logrus.Infof("Inserted %d DKI tickets into the database", len(listOfData))
...
func RefreshTicketDKI() gin.HandlerFunc {
	return func(c *gin.Context) {
		result, err := GetTicketDKIODOOMS()
		if err != nil {
...
func GetLastUpdateTicketDKI() gin.HandlerFunc {
	return func(c *gin.Context) {
		dbWeb := gormdb.Databases.Web

		// Get the latest update timestamp from the database
		var lastUpdatedData time.Time
		if err := dbWeb.Model(&dkimodel.TicketDKI{}).
			Select("updated_at").
...
func TableTicketDKI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
			Draw       int    `form:"draw"`
		...
		t := reflect.TypeOf(dkimodel.TicketDKI{})
		...
		dbWeb := gormdb.Databases.Web
		filteredQuery := dbWeb.Model(&dkimodel.TicketDKI{})
		...
		dbWeb.Model(&dkimodel.TicketDKI{}).Count(&totalRecords)
		...
		var Teknisis []dkimodel.TicketDKI
		query = query.
		...
		for _, person := range Teknisis {
			...
			for i := 0; i < t.NumField(); i++ {
				...
				case "priority":
					...
					case "1":
						ratingHTML = `<i class="bx bxs-star text-danger"></i><i class="bx bx-star"></i><i class="bx bx-star"></i>`
					case "2":
						ratingHTML = `<i class="bx bxs-star text-danger"></i><i class="bx bxs-star text-danger"></i><i class="bx bx-star"></i>`
					case "3":
						ratingHTML = `<i class="bx bxs-star text-danger"></i><i class="bx bxs-star text-danger"></i><i class="bx bxs-star text-danger"></i>`
					default:
						ratingHTML = `<i class="bx bx-star"></i><i class="bx bx-star"></i><i class="bx bx-star"></i>`
					}
...
func GetReportALLTicketDKI() gin.HandlerFunc {
	return func(c *gin.Context) {
		var request struct {
		...
		excelFilename := fmt.Sprintf("(DKI)all_data_report_%d_%s.xlsx", request.UserID, time.Now().Format("02Jan2006_15_04"))
		excelFilePath := filepath.Join(existingPath, excelFilename)

		dbWeb := gormdb.Databases.Web
		var countData int64
		if err := dbWeb.Model(&dkimodel.TicketDKI{}).Count(&countData).Error; err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Error counting DKI tickets: " + err.Error()})
			return
		}

		if countData == 0 {
			c.JSON(http.StatusNotFound, gin.H{"error": "No DKI ticket data available to generate report."})
			return
		}
		...
		styleTitleMaster, err := f.NewStyle(styleExcelTitle("#FF0000", false, "#FFFFFF"))
		if err != nil {
		...
		for batchNum := 0; batchNum < totalBatches; batchNum++ {
			...
			go func(batchNum, offset int) {
				defer wg.Done()
				var batchData []dkimodel.TicketDKI
				err := dbWeb.Model(&dkimodel.TicketDKI{}).
					Order("id ASC").
					Offset(offset).
					Limit(batchSize).
		...
		for batchNum, res := range results {
			if res.Err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("Error fetching DKI tickets (batch %d): %v", batchNum+1, res.Err)})
				return
			}
			batchData := res.Data
			if len(batchData) == 0 {
		...
		NewExcelFilename := fmt.Sprintf("Report_All_DKI_Ticket_%s.xlsx", time.Now().Format("02Jan2006_15_04"))
...
func GetReportTicketDKIFiltered() gin.HandlerFunc {
	return func(c *gin.Context) {
		dbWeb := gormdb.Databases.Web
		...
		for batchNum := 0; batchNum < fetchTotalBatches; batchNum++ {
			...
			go func(batchNum, offset int) {
				...
				q := dbWeb.Model(&dkimodel.TicketDKI{})
		...
		excelFilename := fmt.Sprintf("(DKI)filtered_data_report_%d_%s.xlsx", request.UserID, time.Now().Format("02Jan2006_15_04"))
		excelFilePath := filepath.Join(existingPath, excelFilename)
		if len(dbData) == 0 {
			c.JSON(http.StatusNotFound, gin.H{"error": "No DKI ticket data available to generate report."})
			return
		...
		NewExcelFilename := fmt.Sprintf("(Filtered)Report_DKI_Ticket_%s.xlsx", time.Now().Format("02Jan2006_15_04"))
		c.Header("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
		c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=%s", NewExcelFilename))
		c.Status(http.StatusOK)
