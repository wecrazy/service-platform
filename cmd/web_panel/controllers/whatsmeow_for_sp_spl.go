...
var (
	WhatsappEventsMsgForSPSPL     *events.Message
	WhatsappEventsReceiptForSPSPL *events.Receipt
)

func handleRepliesReactionsAndEditedMsgForSPSPL(forProject string) {
	...
	if ctxInfo != nil && ctxInfo.QuotedMessage != nil && ctxInfo.StanzaID != nil && *ctxInfo.StanzaID != "" {
		...
		// For the notification, we need details from the parent SPLGotSP record
		var dbData sptechnicianmodel.SPLGotSP
		if spMessage.SPLGotSPID != nil {
			if err := dbWeb.First(&dbData, *spMessage.SPLGotSPID).Error; err == nil {
				sentAt := "N/A"
				...
				idText := fmt.Sprintf("SP-%d untuk SPL %s yang dikirim pada %v mendapatkan respon dari %s, yakni %s",
				...
				enText := fmt.Sprintf("SP-%d for SPL %s sent at %v received a reply from %s, which is %s",
					spMessage.NumberOfSP,
					dbData.SPL,
					sentAt,
					userReplied,
				...
				textID := fmt.Sprintf("🙏🏽 Terimakasih atas respon Anda untuk SP-%d SPL %s yang dikirim pada %v.",
					spMessage.NumberOfSP,
					dbData.SPL,
					sentAt,
				)
				textEN := fmt.Sprintf("🙏🏽 Thank you for your reply to SP-%d for SPL %s sent at %v.",
					spMessage.NumberOfSP,
					dbData.SPL,
					sentAt,
				)
...
func handleReceiptForSPSPL(forProject string) {
	e := WhatsappEventsReceiptForSPSPL
	dbWeb := gormdb.Databases.Web
	whatSP := "SP_SPL"
...
func sendLangDocumentMessageForSPSPL(forProject, spl, jid, idmsg, enmsg, lang, filePath string, spNumber int, spSentTo string) {
	...
	sendDocumentViaBotForSPSPL(forProject, spl, jid, msg, filePath, spNumber, spSentTo)
}

func sendDocumentViaBotForSPSPL(forProject, spl, jid, message, filePath string, spNumber int, spSentTo string) {
	dbWeb := gormdb.Databases.Web
	...
	go func() {
		// 1. Find the parent SPLGotSP record
		var dbData sptechnicianmodel.SPLGotSP
		if err := dbWeb.
			Where("for_project = ? AND spl = ?", forProject, spl).
			First(&dbData).
			Error; err != nil {
			logrus.Errorf("Failed to find SPLGotSP for %s/%s: %v", forProject, spl, err)
			return
		}
		...
		if err := dbWeb.Create(&newSPMsg).Error; err != nil {
			logrus.Errorf("Failed to create SPWhatsAppMessage record for %s/%s: %v", forProject, spl, err)
			return
		}

		logrus.Infof("Sucessfully saved SP %d WhatsApp message (that sent to %s) record for %s/%s", spNumber, spSentTo, forProject, spl)
	}()
}
