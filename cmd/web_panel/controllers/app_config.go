...
func TableAppConfig() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		filteredQuery := dbWeb.Model(&model.AppConfig{})
		...
		dbWeb.Model(&model.AppConfig{}).Count(&totalRecords)
		...
		for _, dataInDB := range Dbdata {
			...
			for i := 0; i < t.NumField(); i++ {
				...
				newData[theKey] = renderField(theKey, fieldValue, dataInDB)
			}
...
// renderField handles the rendering of different field types for the AppConfig table
func renderField(fieldKey string, fieldValue reflect.Value, dataInDB model.AppConfig) interface{} {
	switch fieldKey {
	case "id":
		return renderIDField(fieldValue)

	case "created_at", "updated_at", "deleted_at":
		return renderTimeField(fieldValue)

	case "role_id":
		return renderRoleField(fieldValue, dataInDB)

	case "app_name":
		return renderAppNameField(fieldValue)

	case "app_logo":
		return renderAppLogoField(fieldValue)

	case "app_version", "version_no", "version_name":
		return renderVersionField(fieldKey, fieldValue)

	case "version_code":
		return renderVersionCode(fieldValue)

	case "is_active":
		return renderActiveField(fieldValue)

	case "description":
		return renderDescriptionField(fieldValue)

	default:
		return renderDefaultField(fieldValue)
	}
}

// renderIDField renders the ID field with a badge
func renderIDField(fieldValue reflect.Value) string {
	id := fieldValue.Interface().(uint)
	return fmt.Sprintf(`<span class="badge bg-info text-white fw-bold">#%d</span>`, id)
}

// renderTimeField renders timestamp fields
func renderTimeField(fieldValue reflect.Value) interface{} {
...
// renderRoleField renders the role field with enhanced card display
func renderRoleField(fieldValue reflect.Value, dataInDB model.AppConfig) string {
...
// renderAppNameField renders the app name with enhanced styling
func renderAppNameField(fieldValue reflect.Value) string {
...
// renderAppLogoField renders the app logo with clickable image preview modal
func renderAppLogoField(fieldValue reflect.Value) string {
...
// renderVersionField renders version fields with enhanced styling
func renderVersionField(fieldKey string, fieldValue reflect.Value) string {
...
// renderVersionCode renders the version code with enhanced styling
func renderVersionCode(fieldValue reflect.Value) string {
...
// renderActiveField renders the is_active field with enhanced status display
func renderActiveField(fieldValue reflect.Value) string {
...
// renderDescriptionField renders description with truncation
func renderDescriptionField(fieldValue reflect.Value) string {
...
// renderDefaultField handles fallback rendering for unspecified fields
func renderDefaultField(fieldValue reflect.Value) interface{} {
