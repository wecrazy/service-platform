...
function showJsonListModal(button) {
  const title = button.dataset.title;
  const jsonStr = button.dataset.json;
  const url = button.dataset.url || '';
  try {
    const arr = JSON.parse(jsonStr);
    let listHtml = "<ul class=\"list-group\">";
    arr.forEach(item => {
      if (url && title.includes("wo_number")) {
        listHtml += `<li class="list-group-item"><a href="${url}?wo_number=${item}" target="_blank">${item}</a></li>`;
      } else {
    ...
    if (!$("#jsonListModal").length) {
      $("body").append(`
        <div id="jsonListModal" class="iziModal">
          <header>
            <h2>${title}</h2>
          </header>
          <section>
            <div id="jsonListContent"></div>
          </section>
        </div>
      `);
      $("#jsonListModal").iziModal({
        width: 600,
        padding: 20,
        closeButton: true,
        zindex: 9999,
    ...
    $("#jsonListContent").html(listHtml);
    $("#jsonListModal").iziModal("setTitle", title);
    $("#jsonListModal").iziModal("open");
  } catch (e) {
