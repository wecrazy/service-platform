...
const PM_CHART_ID_BNI = 'pie-chart-pm-bni';
const NON_PM_CHART_ID_BNI = 'bar-chart-non-pm-bni';

// Declare pivot container IDs
const tbPMDetailsClassBNI = "dt_data_pm_bni";
const pvtPMBNIID = "pivot-pm-bni";
const tbNonPMDetailsClassBNI = "dt_data_non_pm_bni";
const pvtNonPMBNIID = "pivot-non-pm-bni";
...
async function refreshDataBNI(endpoint, lastUpdateClass, tbClass, btnRefreshClass) {
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    GetLastUpdateBNIData(lastUpdateEndpoint, lastUpdateClass);
                },
...
function GetLastUpdateBNIData(endpoint, lastUpdateClass) {
    $.ajax({
        url: endpoint,
        type: "GET",
        dataType: "json",
...
function initPivotPMBNI(endpoint, dataReq) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#" + pvtPMBNIID).html(tableHTML);

            // Add click event handler for vendor bank filtering
            $("#" + pvtPMBNIID).off('click', '.vendor-bank-filter').on('click', '.vendor-bank-filter', function (e) {
                e.preventDefault();

                const vendorBank = $(this).data('vendor');

                // Find the source input field
                const sourceInput = $('#ft_dt_data_pm_bni_source');
                ...
                if (sourceInput.length > 0) {
                    ...
                    const pmTable = $('.dt_data_pm_bni').DataTable();
                    if (pmTable) {
                    ...
                    $('html, body').animate({
                        scrollTop: $('.dt_data_pm_bni').offset().top - 100
                    }, 800);
                } else {
                    Swal.fire({
                        icon: 'warning',
            ...
            const pieChartContainer = $(`#${PM_CHART_ID_BNI}`).parent();
            ...
            function updatePieChart(selectedBank) {
                ...
                const chart = Highcharts.chart(PM_CHART_ID_BNI, {
                    chart: {
                        type: 'pie',
                        height: 400
                    },
                ...
                if (window.isDark) {
                    $(`#${PM_CHART_ID_BNI}`).removeClass('highcharts-light').addClass('highcharts-dark');
                } else {
                    $(`#${PM_CHART_ID_BNI}`).removeClass('highcharts-dark').addClass('highcharts-light');
                }
...
function initPivotNonPMBNI(endpoint, dataReq) {
    // Ensure required DataTables fields are numeric
    dataReq.draw = parseInt(dataReq.draw) || 1;
    dataReq.start = parseInt(dataReq.start) || 0;
    dataReq.length = parseInt(dataReq.length) || 10;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            const mappedData = jsonData.map(item => ({
                ...
                "Pending Merchant / Bucket Business BNI": parseInt(item.pending_merchant_or_bucket_business) || 0
            }));
                    ...
                    <table class="table table-bordered small" style="font-size: 0.85rem;">
                        <thead>
                            <tr style="background-color: #f8f9fa;">
                                ...
                                <th rowspan="2" style="vertical-align: middle; background-color: #f0f0f0; color: black; text-align: center;">Pending Merchant /<br>Bucket Business BNI</th>
                            </tr>
                            <tr style="background-color: #f8f9fa;">
            ...
            mappedData.forEach(item => {
                ...
                totals.pendingMerchantOrBucketBusiness += item["Pending Merchant / Bucket Business BNI"];
                ...
                tableHTML += `
                    <tr>
                        ...
                        <td class="text-center" style="background-color: #f0f0f0; color: black;">${item["Pending Merchant / Bucket Business BNI"].toLocaleString()}</td>
                    </tr>
                `;
            });
            ...
            Highcharts.chart(NON_PM_CHART_ID_BNI, {
                chart: {
                    type: 'column',
            ...
            // Apply theme classes after chart creation
            if (window.isDark) {
                $(`#${NON_PM_CHART_ID_BNI}`).removeClass('highcharts-light').addClass('highcharts-dark');
            } else {
                $(`#${NON_PM_CHART_ID_BNI}`).removeClass('highcharts-dark').addClass('highcharts-light');
            }
            ...
            $(document).off('click', '.technician-filter-link').on('click', '.technician-filter-link', function (e) {
                ...
                let technicianInput = $('#ft_dt_data_non_pm_bni_technician');
                let taskTypeInput = $('#ft_dt_data_non_pm_bni_task_type');
                ...
                // Also try to apply DataTable column search if table exists
                const nonPmTable = $('.dt_data_non_pm_bni').DataTable();
                if (nonPmTable) {
                ...
                if ($('.' + tbNonPMDetailsClassBNI).length > 0) {
                    $('html, body').animate({
                        scrollTop: $('.' + tbNonPMDetailsClassBNI).offset().top - 100
                    }, 500);
                }
            });
...
function downloadExcelReportPMBNI(userId, endpoint, reportType) {
    ...
    if (reportType === 'Filtered') {
        ...
        var storedFilters = localStorage.getItem('datatableFiltersPMBNI');
        if (storedFilters) {
            try {
                ...
                var currentSearch = $('#dt_data_pm_bni_wrapper .dataTables_filter input').val();
                if (currentSearch) {
    ...
    Swal.fire({
        ...
        text: reportType + ' PM BNI report generation started. Button will be restored shortly.',
        timer: 2000,
        showConfirmButton: false
    });
...
function downloadExcelReportNonPMBNI(userId, endpoint, reportType) {
    ...
    if (reportType === 'Filtered') {
        // Get stored filter data from localStorage 
        var storedFilters = localStorage.getItem('datatableFiltersNonPMBNI');
        if (storedFilters) {
            try {
                var filters = JSON.parse(storedFilters);
                // Add all filter data to form
                ...
                console.error('Error parsing stored filters:', e);
                // Fallback to just search value
                var currentSearch = $('#dt_data_non_pm_bni_wrapper .dataTables_filter input').val();
                if (currentSearch) {
                    formData['search[value]'] = currentSearch;
                }
            }
    ...
    Swal.fire({
        icon: 'info',
        title: 'Download Initiated',
        text: reportType + ' Non-PM BNI report generation started. Button will be restored shortly.',
        timer: 2000,
        showConfirmButton: false
    });
