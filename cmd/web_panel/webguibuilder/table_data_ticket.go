...
func TABLE_DATA_TICKET(session string, redisDB *redis.Client, db *gorm.DB) template.HTML {
	var tableHeaders []webgui.Column
	dbData := model.TicketHommyPayCC{}
	...
	templates := webgui.RenderDataTableServerSideDataTicket(
		"Data Ticket",
		"dt_data_ticket",
		fun.GLOBAL_URL+"web/"+fun.GetRedis("web:"+session, redisDB)+"/tab-hommy-pay-cc-ticket/table",
		5,
