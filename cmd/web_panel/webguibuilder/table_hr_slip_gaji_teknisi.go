...
func TABLE_SLIP_GAJI_TEKNISI_EDC(session string, redisDB *redis.Client) template.HTML {
	...
	templates := webgui.RenderDataTableServerSideSlipGajiTeknisiEDC(
		"Detail Data",
		"dt_slip_gaji_teknisi_edc",
		fun.GLOBAL_URL+"web/"+fun.GetRedis("web:"+session, redisDB)+"/tab-hr-slip-gaji-teknisi/table_edc",
		5,
...
func TABLE_SLIP_GAJI_TEKNISI_ATM(session string, redisDB *redis.Client) template.HTML {
	var tableHeaders []webgui.Column
	db := gormdb.Databases.Web
	...
	templates := webgui.RenderDataTableServerSideSlipGajiTeknisiATM(
		"Detail Data",
		"dt_slip_gaji_teknisi_atm",
		fun.GLOBAL_URL+"web/"+fun.GetRedis("web:"+session, redisDB)+"/tab-hr-slip-gaji-teknisi/table_atm",
		5,
		[]int{5, 10, 25, 50, 100, 250},
		[]any{[]any{1, "asc"}},
		tableHeaders,
