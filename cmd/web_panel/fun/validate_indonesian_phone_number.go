...
func SanitizePhoneNumber(phone string) (string, error) {
	...
	for _, phoneNum := range phoneNumbers {
		sanitized, err := sanitizeSinglePhoneNumber(phoneNum)
		if err == nil {
...
// sanitizeSinglePhoneNumber handles a single phone number sanitization
func sanitizeSinglePhoneNumber(phone string) (string, error) {
...
// SanitizeAllPhoneNumbers returns all valid phone numbers from a string containing multiple numbers
// This is useful if you want to get all valid numbers instead of just the first one
func SanitizeAllPhoneNumbers(phone string) ([]string, error) {
	// Split the input by common delimiters to handle multiple phone numbers
	delimiters := []string{"/", ",", ";", "|", " / ", " , ", " ; ", " | ", "atau", " atau ", "or", " or "}
	phoneNumbers := []string{phone} // Start with the original string
