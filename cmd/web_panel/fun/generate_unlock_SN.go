...
func encrypt3DESCBCNoPadding(key, data, iv []byte) ([]byte, error) {
	...
	if len(key) != 16 && len(key) != 24 {
		return nil, errors.New("key length must be either 16 or 24 bytes for 3DES")
	}
	...
	if len(data)%block.BlockSize() != 0 {
		return nil, fmt.Errorf("data length must be a multiple of %d bytes", block.BlockSize())
	}
...
func xorStrings(str1, str2 string) (string, error) {
	...
	if len(bytes1) != len(bytes2) {
		return "", errors.New("strings must be of the same length to XOR")
	}
