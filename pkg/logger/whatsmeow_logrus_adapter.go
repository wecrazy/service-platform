...
// LogrusAdapter adapts logrus to the waLog.Logger interface used by Whatsmeow
type LogrusAdapter struct {
	logger *logrus.Entry
}

// Debugf logs a debug-level message via the underlying logrus entry.
func (a *LogrusAdapter) Debugf(msg string, args ...interface{}) { a.logger.Debugf(msg, args...) }

// Infof logs an info-level message via the underlying logrus entry.
func (a *LogrusAdapter) Infof(msg string, args ...interface{}) { a.logger.Infof(msg, args...) }

// Warnf logs a warn-level message via the underlying logrus entry.
func (a *LogrusAdapter) Warnf(msg string, args ...interface{}) { a.logger.Warnf(msg, args...) }

// Errorf logs an error-level message via the underlying logrus entry.
