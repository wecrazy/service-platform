// Package observability provides utilities for setting up OpenTelemetry tracing with Tempo as the backend.
// It includes functions to initialize the tracer provider, get the global tracer provider, and gracefully shutdown the tracer.
// The InitTracer function sets up the OTLP gRPC exporter to send traces to Tempo, and configures the tracer provider with service information and sampling settings.
// The GetTracerProvider function allows other parts of the application to access the global tracer provider for creating tracers and spans.
// The ShutdownTracer function should be called during application shutdown to ensure that all spans are flushed and resources are cleaned up properly.
// For more details on how to use this package, see the documentation in the internal/observability package and the main application README.
...
var globalTracerProvider *trace.TracerProvider // Global tracer provider instance
