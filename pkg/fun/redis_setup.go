...
// StartRedisWindows attempts to start Redis on Windows, first trying the native installation and then WSL if available.
...
// StartRedisLinux attempts to start Redis on Linux using the system's service manager or directly if available.
...
// WaitForRedis checks if Redis is accepting connections on the specified host and port, retrying for a given number of attempts.
...
// GetRedis retrieves the value associated with the specified key from Redis using the provided Redis client.
