...
func RemoveOldFilesInNeedsDir(olderThan string) {
	...
	logrus.Infof("Removing directories older than %s (cutoff: %s) from needs directories", olderThan, cutoffTime.Format(config.DateYYYYMMDD))
...
func removeOldDateDirectories(dir string, cutoffTime time.Time) (int, int, int64, error) {
	...
	for _, entry := range entries {
		...
		dirDate, err := time.Parse(config.DateYYYYMMDD, entry.Name())
		if err != nil {
		...
		if dirDate.Before(cutoffTime) {
			...
			if err != nil {
				...
				logrus.Infof("Deleted directory: %s (date: %s, files: %d, size: %s)",
					entry.Name(), dirDate.Format(config.DateYYYYMMDD), fileCount, formatBytes(dirSize))
				dirsDeleted++
			...
			logrus.Debugf("Keeping directory: %s (date: %s, newer than cutoff)",
				entry.Name(), dirDate.Format(config.DateYYYYMMDD))
		}
	}
...
func calculateDirSize(dir string) (int, int64) {
	...
	filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		_ = path // for now we don't need the path, but we could log it if needed
...
func checkForDateDirectories(dir string) bool {
	...
	for _, entry := range entries {
		...
		// Try to parse directory name as date
		_, err := time.Parse(config.DateYYYYMMDD, entry.Name())
		if err == nil {
		...
		if info.ModTime().Before(cutoffTime) {
			...
			if err != nil {
				...
				logrus.Debugf("Deleted file: %s (size: %s, modified: %s)",
					entry.Name(), formatBytes(fileSize), info.ModTime().Format(config.DateYYYYMMDDHHMMSS))
				filesDeleted++
				sizeFreed += fileSize
			}
		}
