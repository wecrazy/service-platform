...
if podman ps --filter "name=service-platform-prometheus" --format "{{.Names}}" | grep -q "service-platform-prometheus"; then
    echo "📊 Prometheus is running, checking data size..."
    PROMETHEUS_SIZE_BEFORE=$(podman exec service-platform-prometheus du -sh /prometheus 2>/dev/null | cut -f1 || echo "N/A")
...
if podman ps --filter "name=service-platform-prometheus" --format "{{.Names}}" | grep -q "service-platform-prometheus"; then
    ...
    echo "ℹ️  Prometheus is not running, skipping cleanup"
fi
